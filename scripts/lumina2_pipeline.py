...
class Lumina2_Pipeline_DoE(DiffusionPipeline, Lumina2LoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        self.mask_processor  = VaeImageProcessor(vae_scale_factor=self.vae_scale_factor * 2,  
    ...
    def __call__(
        ...
        if image is not None:
            ...
            if strength < 1.0:
                ...
                latents = self.scheduler.scale_noise(image_latents, ts, noise)

            if mask_image is not None:
                # 5.1. Prepare masked latent variables
                mask = self.mask_processor.preprocess(mask_image.resize((width//8, height//8))).to(device='cuda')
                mask = mask / mask.max()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if doDiffDiff and float((i+1) / self.num_timesteps) <= mask_cutoff:
                    tmask = (mask >= float((i+1) / self.num_timesteps))
                    ts = torch.tensor([t], device='cuda')
                    ts = ts[:1].repeat(num_images_per_prompt)
                    init_latents_proper = self.scheduler.scale_noise(image_latents, ts, noise)
